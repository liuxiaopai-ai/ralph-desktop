<script lang="ts">
  ...
  const e2eProjectPath = import.meta.env.VITE_E2E_PROJECT_PATH as
    | string
    | undefined;
  ...
  const shortcuts = $derived([
    {
      ...
      description: $_("shortcuts.newProject"),
    },
    {
      key: ",",
      ctrl: true,
      action: () => (showSettings = true),
      description: $_("shortcuts.openSettings"),
    },
    {
      key: "?",
      ctrl: true,
      action: () => (showShortcuts = true),
      description: $_("shortcuts.showHelp"),
    },
    {
      key: "Escape",
      action: handleEscape,
      description: $_("shortcuts.closeDialog"),
    },
  ]);
  ...
  async function loadProjectDetails(id: string) {
    try {
      ...
      // Update loop state with logs
      const logEntries = logs.map((line) => {
        const match = line.match(
          /^\[#(\d+)\] (\d{2}:\d{2}:\d{2}) \[(OUT|ERR)\] (.*)$/,
        );
        if (match) {
          const now = new Date();
      ...
      // We need to access loop store methods more directly or assume they are available
      // Since loop.ts exports functions, we can import and use them.
  ...
  const availableCliCount = $derived(
    $availableClis.filter((c) => c.available).length,
  );
</script>
  ...
  <div class="flex flex-1 min-h-0">
    ...
    <div class="w-64 bg-vscode-sidebar border-r border-vscode flex flex-col">
      ...
      <div class="px-4 py-3 flex items-center justify-between">
        <div>
          <h1 class="text-sm font-semibold text-vscode uppercase tracking-wide">
            {$_("app.name")}
          </h1>
        </div>
        <button
      ...
      <div class="px-3 pb-2">
        ...
        {#if availableCliCount === 0}
          <p class="text-xs text-[#f14c4c] mt-2 text-center">
            {$_("app.noCliDetected")}
          </p>
        {/if}
      </div>
      ...
      <div class="px-3 py-2 border-t border-vscode text-xs text-vscode-muted">
        <div class="flex justify-between">
          <span
            >{$availableClis.find((c) => c.available)?.name ||
              $_("app.noCli")}</span
          >
          <span
            >{$_("app.projectsCount", {
      ...
      {:else}
        ...
        <div class="flex-1 flex items-center justify-center">
          <div class="text-center text-vscode-dim">
            ...
            <h2 class="text-base font-medium mb-1 text-vscode">
              {$_("app.selectOrCreate")}
            </h2>
